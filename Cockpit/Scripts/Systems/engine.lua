...
local throttle_rate                  = get_plugin_option_value("A-4E-C", "throttleRate", "local") *
    0.02 -- THROTTLE RATE, 1.0 to 5.0 percent, increments of 0.1, defined in the special menu, to a percentage change to apply
local throttle_accelerating          = 0
local throttle_acceleration_default  = get_plugin_option_value("A-4E-C", "throttleAcceleration", "local") *
    0.0002                               -- THROTTLE ACCELERATION tuning
local throttle_accelerating_duration = 0 --additional acceleration from cumulative duration
local throttle_acceleration_rate     =
    throttle_acceleration_default        -- set default and await input
...
function SetCommand(command, value)
    ...
    elseif command == device_commands.throttle_click_ITER then
        ...
        if value == -1 or value == 1 then
            ...
            if throttle_state == THROTTLE_OFF then
                current_throttle_click_position = -1
            elseif throttle_state == THROTTLE_IGN then
                current_throttle_click_position = 0
            ...
            if new_throttle_click_value > 1 then
                new_throttle_click_value = 1
            elseif new_throttle_click_value < -1 then
                new_throttle_click_value = -1
            end
    ...
    elseif command == Keys.FuelControl then
        dev:performClickableAction((device_commands.ENGINE_fuel_control_sw), ((manual_fuel_control_mode_sw * -1) + 1),
            false)
    else
...
function update()
    ...
    if throttle_accelerating ~= 0 then
        ...
        dispatch_action(nil, iCommandPlaneThrustCommon,
            -2.0 * throttle + 1.0 - throttle_accelerating * throttle_accelerating_duration)
