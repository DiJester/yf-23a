...
--dofile(folder .. "../inpututils.lua")

local res = external_profile("Config/Input/Aircrafts/base_keyboard_binding.lua")
ignore_features(res.keyCommands, {
    "dragchute", "shiptakeoff", "a2g"
})
--Ignore_names(res.keyCommands, { _('Aircraft Rudder Left'), _('Aircraft Rudder Right') })
--Ignore_names(res.axisCommands, { _('Rudder') })
...
join(res.keyCommands, {
    ...
    { combos = defaultDeviceAssignmentFor("unlock_target"),              down = iCommandSensorReset,                                    name = _('Radar - Return To Search/NDTWS'),                  category = _('Sensors') },
    ...
    { combos = { { key = 'Z' } },                                        down = device_commands.left_rudder_start,                      up = device_commands.left_rudder_stop,                       cockpit_device_id = devices.RUDDER,      name = _('Aircraft Rudder Left (Key)'),  category = { _('Flight Control'), _('YF23A') } },
    { combos = { { key = 'X' } },                                        down = device_commands.right_rudder_start,                     up = device_commands.right_rudder_stop,                      cockpit_device_id = devices.RUDDER,      name = _('Aircraft Rudder Right (Key)'), category = { _('Flight Control'), _('YF23A') } },
})
return res
